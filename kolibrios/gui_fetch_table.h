...
struct nsurl* kolibri_get_resource_url(const char *path); /* Can be used to always load some .html file probably */

nserror kolibri_get_resource_data(const char *path, const uint8_t **data, size_t *data_len);
nserror kolibri_release_resource_data(const uint8_t *data); /*get_ and release_ functions work in unison one after another*/
