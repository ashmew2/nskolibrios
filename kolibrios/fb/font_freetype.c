...
fb_new_face(const char *option, const char *resname, const char *fontname)
{
	...
	char buf[PATH_MAX];
		...
		filepath_sfind(respaths, buf, fontname);
                ...
                newf->fontfile = "/usbhd0/1/kolibrios/fb/res/sans.ttf";
		//strdup(buf);
        }
...
bool fb_font_init(void)
{
	...
	fb_faceid_t *fb_face;
        ...
        if (error) {
		//debug_board_write_str("Freetype could not initialised (code X)\n");
	...
	if (max_cache_size < CACHE_MIN_SIZE) {
        ...
        error = FTC_Manager_New(library, 
                                ...
                                &ft_cmanager);
        if (error) {
		//debug_board_write_str("Freetype could not initialise cache manager (code %d)\n");
	...
	if (fb_face == NULL) {
		/* The sans serif font is the default and must be found. */
                //debug_board_write_str("Could not find the default font\n");
