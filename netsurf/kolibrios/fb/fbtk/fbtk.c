...
/* static void debug_board_write_byte(const char ch){ */
...
/* static void debug_board_write_str(const char* str){ */
/*   while(*str) */
...
do_redraw(nsfb_t *nsfb, fbtk_widget_t *widget)
{
	...
	//debug_board_write_str("check if the widget requires redrawing\n");
	/* if (widget == NULL) */
	/* 	debug_board_write_str("widget is NULL now!\n"); */

	/* check if the widget requires redrawing */
	if (widget->redraw.needed == true) {
		//debug_board_write_str("Widget redraw needed.\n");
		...
		if (nsfb_plot_set_clip(nsfb, &plot_ctx) == true) {
			//debug_board_write_str("posting callback ftk_post_callback\n");
			fbtk_post_callback(widget, FBTK_CBT_REDRAW);
			//debug_board_write_str("posting callback done.\n");
	...
	if (widget->redraw.child) {
		//debug_board_write_str("Redrawing child\n");
		cwidget = widget->last_child;
		while (cwidget != NULL) {
	...
	//debug_board_write_str("Returning from do_redraw\n");
	return 1;
}
...
fbtk_redraw(fbtk_widget_t *widget)
{
	...
	/* ensure we have the root widget */
	root = fbtk_get_root_widget(widget);
	//debug_board_write_str("Calling do_redraw inside fbtk_redraw()\n");
...
{
	//debug_board_write_str("Inside fbtk_post_callbackwidget is NUll in post callback\n");

	fbtk_callback_info cbi;
	int ret = 0;
	va_list ap;

	//debug_board_write_str("widget is NUll in post callback\n");
	...
	if (widget->mapped == false)
		{
		//debug_board_write_str("widget is not mappedin post callback\n");
		return ret;
		}

	if (widget->callback[cbt] != NULL) {
		//debug_board_write_str("widget has a callback available in post callback\n");
		cbi.type = cbt;
		...
		//debug_board_write_str("Calling widget callback in post callback\n");
		ret = (widget->callback[cbt])(widget, &cbi);
		//debug_board_write_str("widget callback returned in post callback\n");
	}
