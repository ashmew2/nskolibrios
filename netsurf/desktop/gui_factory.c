...
nserror netsurf_register(struct netsurf_table *gt)
{
	...
	if (guit != NULL) {
	        debug_board_write_str("guit is NULL. return NSERROR_INIT_FAILED\n");	  
		return NSERROR_INIT_FAILED;
	}

	/* check table is present */
	if (gt == NULL) {
	        debug_board_write_str("gt is NULL. return NSERROR_BAD_PARAMETER\n");	  
		return NSERROR_BAD_PARAMETER;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("browser table failed to initialize\n");
		return err;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("window table failed to initialize\n");
		return err;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("fetch table failed to initialize\n");
		return err;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("bitmap table failed to initialize\n");
		return err;
	}

	/* file table */
	if (gt->file == NULL) {
	  	debug_board_write_str("NULL is FILE TABLE!");
		gt->file = default_file_table;
	}

	err = verify_file_register(gt->file);
	if (err != NSERROR_OK) {
	        debug_board_write_str("file table failed to initialize\n");
		return err;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("download table failed to initialize\n");
		return err;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("clipboard table failed to initialize\n");
		return err;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("utf8 table failed to initialize\n");
		return err;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("search table failed to initialize\n");
		return err;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("search_web table failed to initialize\n");
		return err;
	}
	...
	if (err != NSERROR_OK) {
	        debug_board_write_str("llcache table failed to initialize\n");
		return err;
	}
