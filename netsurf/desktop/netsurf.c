...
{
        debug_board_write_str("netsurf_init..\n");
	nserror ret;
	...
	if (ret != NSERROR_OK) {
	  debug_board_write_str("corestrings_init failed!\n");
	  return ret;
	}
	...
	debug_board_write_str("Now setting up internal caches etc for Netsurf \n");
	/* image handler bitmap cache */
	ret = image_cache_init(&image_cache_parameters);
	if (ret != NSERROR_OK) {
	  debug_board_write_str("image_cache_init failed! \n");
	  return ret;
	}
	...
	ret = nscss_init();
	if (ret != NSERROR_OK) {
	  debug_board_write_str("nscss_init failed! \n");
	  return ret;
	}

	ret = html_init();
	if (ret != NSERROR_OK) {
	  debug_board_write_str("html_init failed! \n");
	  return ret;
	}

	ret = image_init();
	if (ret != NSERROR_OK) {
	  debug_board_write_str("image_init failed! \n");
	  return ret;
	}

	ret = textplain_init();
	if (ret != NSERROR_OK) {
	  debug_board_write_str("textplain_init failed! \n");
	  return ret;
	}
	...
	ret = mimesniff_init();
	if (ret != NSERROR_OK) {
	  debug_board_write_str("mimesniff_init failed! \n");
	  return ret;
	}
	...
	ret = fetcher_init();
	if (ret != NSERROR_OK) {
	  debug_board_write_str("fetcher_init failed! \n");
	  return ret;
	}
	...
	if (ret != NSERROR_OK) {
	  debug_board_write_str("hlcache_initialise failed! \n");
	  return ret;
	}
	...
	if (ret != NSERROR_OK) {
	  debug_board_write_str("ns_system_colour_init failed! \n");
	  return ret;
	}
