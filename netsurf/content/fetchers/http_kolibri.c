...
/* XXXXXXXXXXXXX */
  ...
  if(header_count!=0) { /* We found some headers */
    ...
    for(j = 0; headers[j] != NULL; j++) {
      debug_board_write_str(headers[j]);
	...
	if(poller->transfer_direction == RECEIEVE)
	  {
		...
		for(i = 0, newline = 0;i < header_length; i++)
		  {
			...
			/* debug_board_printf("Feeding: %s", msg.data.header_or_data.buf); */
		...
		fetch_send_callback(&msg, poller->fetch_handle);
	      ...
	      char *location = http_find_header_field_asm(poller->http_handle, "location");
	      ...
	      for(ptr = location;; ptr++)
		{
	      ...
	      redirected_url = malloc(length+1);

	      assert(redirected_url != NULL);
		
	      strncpy(redirected_url, location, length+1);
	      redirected_url[length] = '\0';
		
	      debug_board_write_str("Redirecting: ");
	      debug_board_write_str(redirected_url);
	      debug_board_write_str("\n");
	      //		__asm__ __volatile__("int3");
	      /* SET HTTP CODE HERE! FIXME */		
	      msg.type = FETCH_REDIRECT;
	      msg.data.redirect = redirected_url;
	      fetch_send_callback(&msg, poller->fetch_handle);
	    }
	    ...
	    /* else if (f->only_2xx && strncmp(nsurl_access(f->url), "http", 4) == 0 && */
