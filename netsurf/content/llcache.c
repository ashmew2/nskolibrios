...
static nserror llcache_object_refetch(llcache_object *object)
{
	...
	/* Kick off fetch */
	debug_board_write_str("llcache.c: Calling fetch_start\n");
...
#undef REDIRECT_LIMIT
	...
	debug_board_write_str("Before nsurl_join : ");
	debug_board_write_str(target);
	...
	/* ; */
	LOG("Redirect url: %s", nsurl_access(url));
...
{
  /* char data_addr[40]; */
  /* debug_board_write_str(data_addr, "a=%p and len=%u", (void *)data, len); */
  /* ; */
...
static void llcache_fetch_callback(const fetch_msg *msg, void *p)
{
	...
	case FETCH_REDIRECT:
		...
		debug_board_write_str("In llcache.\n");
		debug_board_write_str(msg->data.redirect);
		//		__asm__ __volatile__("int3");
	...
	case FETCH_DATA:
		/* Received some data */
	  debug_board_write_str("Handling FETCH_DATA in llcache cb");
	  sprintf(xx, "addr: %p , len = %u\n", msg->data.header_or_data.buf, msg->data.header_or_data.len);
	  debug_board_write_str(xx);
	  /* ; */
