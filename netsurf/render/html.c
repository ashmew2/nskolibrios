	    ...
	    struct content **c)
{
	...
	LOG("Inside html_create function");
	...
	if (error != NSERROR_OK) {
	  LOG("html_create_html_data failed failed");
	  //	  __asm__ __volatile__("int3");
	...
	if (error != NSERROR_OK) {
	  LOG("html_css_new_stylesheets failed");
	  //	  __asm__ __volatile__("int3");
		content_broadcast_errorcode(&html->base, error);
		free(html);
		return error;
	...
	*c = (struct content *) html;
	...
	LOG("Returning from html_create");
	//	__asm__ __volatile__("int3");
